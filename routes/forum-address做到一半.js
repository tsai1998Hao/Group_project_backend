...
router.get("/api/detail_article_comment/:article_id", async(req, res)=>{ 
    const article_id=req.params.article_id;
    res.locals.title="編輯|"+ res.locals.title;
    ...
    const sql2 = `SELECT * FROM forum_comment WHERE article_id=?`;
    const [rows2]= await db.query(sql2, [article_id]);
    const row2 =rows2[0];

    res.json({success: true, row, rows2});
});
...
//     const sql2 = `SELECT * FROM forum_comment WHERE article_id=?`;
//     const [rows2]= await db.query(sql, [article_id]);
//     if(!rows.length){
//         return res.redirect(req.baseUrl);
//     }
//     const row2 =rows2[0];
